...
(function ($, window, Modernizr, document, CONFIG) {
	...
	$(function () {
		...
		(function () {
			...
			if ($('.sidebar-tweet, .tmm_tweet').length) {
				twitterFetcher.fetch(CONFIG.fetchTwits);
			}
			
			if ($('.tmm_tweet').length) {

				var $tweet = $('#tweets_bar');

				setTimeout(function(){
					var $tweets = $tweet.children('ul');
	...
	/* ---------------------------------------------------------------------- */
		...
		OnePage.prototype = {
			...
			sectionAnimate: function (section) {
				...
				var touchCorrect = self.touch ? self.header.css('position') == 'relative' ? self.header.height() - 70 : '' : '';

				$('html, body').stop(true, true).animate({
					scrollTop: section.offset().top - touchCorrect
				}, {
